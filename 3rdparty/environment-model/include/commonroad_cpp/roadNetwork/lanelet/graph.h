...
template <class V, class E> class graph {
  ...
  public:
    ...
    int amount_vertices() const { return (std::size(m_vertices) - std::size(m_freed_vertex_ids)); }

    void clear() { *this = graph<V, E>{}; }
    ...
    std::pair<std::ptrdiff_t, std::ptrdiff_t> add_undirected_edge(std::ptrdiff_t from_vertex_id,
                                                                  std::ptrdiff_t to_vertex_id, const E &data) {
    ...
    std::ptrdiff_t edge_source_vertex_id(std::ptrdiff_t edge_id) const { return m_edges[edge_id].source_vertex_id(); }

    std::ptrdiff_t edge_target_vertex_id(std::ptrdiff_t edge_id) const { return m_edges[edge_id].target_vertex_id(); }

    bool is_edge_present(std::ptrdiff_t edge_id) const {
        return is_id_in_use(edge_id, (std::size(m_edges) - 1), m_freed_edge_ids);
    }

    void remove_edge(std::ptrdiff_t edge_id) { m_freed_edge_ids.insert(edge_id); }

    std::ptrdiff_t amount_edges() const { return (std::size(m_edges) - std::size(m_freed_edge_ids)); }
    ...
    bool empty() const { return (amount_vertices() == 0); }
  ...
  private:
    class edge {
      public:
        ...
        std::ptrdiff_t id() const { return m_id; }

        std::ptrdiff_t source_vertex_id() const { return m_source_vertex_id; }

        std::ptrdiff_t target_vertex_id() const { return m_target_vertex_id; }

        const E &data() const { return m_data; }

        void transpose() { std::swap(m_source_vertex_id, m_target_vertex_id); }
    ...
    class vertex {
      public:
        ...
        void add_edge(std::ptrdiff_t edge_id) { m_edge_ids.insert(edge_id); }

        void remove_edge(std::ptrdiff_t edge_id) { m_edge_ids.erase(edge_id); }

        void clear_edges() { m_edge_ids.clear(); }

        std::set<std::ptrdiff_t> edge_ids() const { return m_edge_ids; }

        const V &data() const { return m_data; }
