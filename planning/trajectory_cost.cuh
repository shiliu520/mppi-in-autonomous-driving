 ...
 * @LastEditTime: 2025-11-23 22:31:31
...
inline __device__ float TrajectoryCost::computeStateCost(float* s, int timestep, float* theta_c,
  ...
  const float position_cost = params_.bike_position_coeff * fabsf(s[1] - 3.2);
  const float velocity_cost = params_.bike_velocity_coeff * fabsf(s[2] - params_.target_velocity);
