 ...
 * @LastEditTime: 2025-11-20 23:20:48
 * @FilePath: /mppi-in-autonomous-driving/simulator/cubic_spline.cpp
...
CubicSpline::CubicSpline(vector<double> _x, vector<double> _y) {
  ...
  y = _y;
  nx = x.size();
  h = cubic_spline_utils::vector_diff(x);
...
vector<double> CubicSpline2D::calc_s(vector<double> _x, vector<double> _y) {
  vector<double> dx = cubic_spline_utils::vector_diff(_x);
  vector<double> dy = cubic_spline_utils::vector_diff(_y);
  vector<double> ds;
  vector<double> s = {0};
  for (size_t idx = 0; idx < dx.size(); ++idx) {
    ds.push_back(hypot(dx[idx], dy[idx]));
  }
  vector<double> cum = cubic_spline_utils::vector_cumsum(ds);
